...
object InstitutionLoader extends App {
  ...
  val token         = config.getString("hmda.loader.institution.token")
    ...
    postOrPut match {
      case "put-token" =>
        HttpRequest(uri = s"$url", method = HttpMethods.PUT)
          .withEntity(ContentTypes.`application/json`, ByteString(json)).withHeaders(Authorization(OAuth2BearerToken(token)))
      case "post-token" =>
